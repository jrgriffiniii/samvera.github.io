...
NOTE:
- As generated, model_class is set to the generated model class
- As generated, terms includes basic work terms defined in `app/forms/hyrax/forms/work_form.rb` [terms](https://github.com/samvera/hyrax/blob/v2.1.0.rc2/app/forms/hyrax/forms/work_form.rb#L24-L31) method.
- A controller class was also generated and configures form_class to be the one described here, e.g., `self.form_class = Hyrax::Forms::GenericWorkForm`  See [Understanding the Controller](customize-metadata-controller.html).


## Notable WorkForm methods

This class inherits from [Hyrax::Forms::WorkForm](https://github.com/samvera/hyrax/blob/v2.1.0.rc2/app/forms/hyrax/forms/work_form.rb) which has several methods of note.

| Method | Description | Overridden? |
| ------ | ----------- | ----------- |
| terms | Returns an array of all terms that will be displayed on the form. | Common |
| required_fields | Returns an array of terms that are required.  The form will not save without these fields having a value. | Common |
| primary_terms | Determines which fields will be displayed at the top of the form.  They will be visible when the new/edit form is initially displayed (referred to as being above the fold). By default, this returns the required_fields array. | Common |
| secondary_terms | Returns the array of terms that are optional and will be displayed at the bottom of the form (below the fold).  These fields are hidden until the user clicks `Additional fields` button.  This method also removes fields that will not be filled in by a user for various reasons (e.g. populated by the system, displayed on another tab, set based on data in other fields, etc.). | Common |
...
<ul class='info'><li>See [Other Metadata Customizations](customize-metadata-other-customizations.html) in section [Making a default property single-value](customize-metadata-other-customizations.html#making-a-default-property-single-value) for an example of when you would override `multiple?` and `sanitize_params`.</li></ul>
...
module Hyrax
  ...
  class GenericWorkForm < Hyrax::Forms::WorkForm
    self.model_class = ::GenericWork
    self.terms += [:resource_type, :contact_email, :contact_phone, :department]
    self.required_fields += [:department, :contact_email]  end
end
```
...
- Without additional customization, the field will be a text input field.  (See [HydraEditors's app/views/records/edit_fields/_default.html.erb](https://github.com/samvera/hydra-editor/blob/4.0-stable/app/views/records/edit_fields/_default.html.erb)
- For contact_email and department, because we did **NOT** set `multiple: true` in the model, there will be only a single value set for these properties.
...
TODO: May want to show use of primary_terms and secondary_terms methods. 
...
You can see [more examples](https://github.com/samvera/hyrax/tree/v2.1.0.rc2/app/views/records/edit_fields) by exploring those created for the default fields in Hyrax.

### For a single-value field (optional)

This example shows changing how the contact_email field is created.  It sets the field up as an email input.

Create file `app/views/records/edit_fields/_contact_email.html.erb` and set contents to...
```erb
...
This example shows changing how the contact_phone field is created.  It sets the field up as a phone input.

Create file `app/views/records/edit_fields/_contact_phone.html.erb` and set contents to...
```erb
...
Create `app/views/records/edit_fields/_department.html.erb` and set contents to...
